 ...
 * is equal to 1.
 * 1) For example, the "string" s = "0090089" can be split into ["0090", "089"] 
 * with numerical values [90, 89]. The values are in descending order and 
 * adjacent values differ by 1, so this way is valid.
 * 2) Another example, the "string" s = "001" can be split into ["0", "01"], 
 * ["00", "1"], or ["0", "0", "1"]. How all the ways are invalid because they 
 * have numerical values [0, 1], [0, 1], and [0, 0, 1] respectively, all of 
 ...
 * Explanation: There is no valid way to split s.
 */
package com.halconbit.challenge.java.backtracking.splittingAStringIntoDescendingConsecutiveValues;
